    ...
    def _emit_constraint_catalog(self, tables: List):
        ...
        for t in tables:
            ...
            for c in t.columns:
                ...
                # UNIQUE
                if getattr(c, "unique", None):
                    code = f"ErrorCode.{entity}_{col}".upper().replace("-", "_") + "_DUPLICATE"
                    lines.append(f'        byConstraint.put("uk_{t.name}_{col}".toLowerCase(java.util.Locale.ROOT), {code});')
                # FK
                if getattr(c, "foreign_key_table", None):
                    code = f"ErrorCode.{entity}_{col}".upper().replace("-", "_") + "_FK_VIOLATION"
                    lines.append(f'        byConstraint.put("fk_{t.name}_{col}".toLowerCase(java.util.Locale.ROOT), {code});')
                # NOT NULL
                if getattr(c, "nullable", None) is False:
                    code = f"ErrorCode.{entity}_{col}".upper().replace("-", "_") + "_REQUIRED"
                    lines.append(f'        byConstraint.put("nn_{t.name}_{col}".toLowerCase(java.util.Locale.ROOT), {code});')
