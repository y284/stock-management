...
@RestController
@Validated
@RequestMapping(path = "/api/sale-commande", produces = MediaType.APPLICATION_JSON_VALUE)
public class SaleCommandeController {

    private final SaleCommandeService service;

    public SaleCommandeController(SaleCommandeService service) {
        this.service = service;
    }

    // ===== Read =====
    @GetMapping
    public List<SaleCommandeDto> list() {
        return service.findAll();
    }

    @GetMapping("/page")
    public Page<SaleCommandeDto> listPage(
    ...
    @GetMapping("/sorted")
    public List<SaleCommandeDto> listSorted(Sort sort) {
        return service.findAll(sort);
    }

    @GetMapping("/{id}")
    public SaleCommandeDto get(@PathVariable Long id) {
        return service.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("saleCommande not found with id=" + id));
    }

    @GetMapping("/uuid/{uuid}")
    public SaleCommandeDto getByUuid(@PathVariable UUID uuid) {
        return service.findByUuid(uuid)
                .orElseThrow(() -> new ResourceNotFoundException("saleCommande not found with uuid=" + uuid));
    }

    // ===== Create =====
    @PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<SaleCommandeDto> create(@Valid @RequestBody SaleCommandeDto dto) {
        SaleCommandeDto created = service.create(dto);
    ...
    public SaleCommandeDto update(@PathVariable Long id, @Valid @RequestBody SaleCommandeDto dto) {
        return service.update(id, dto);
    }

    // ===== Patch (partial) =====
    @PatchMapping(path = "/{id}", consumes = MediaType.APPLICATION_JSON_VALUE)
    public SaleCommandeDto patch(@PathVariable Long id, @RequestBody SaleCommandeDto dto) {
