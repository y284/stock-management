...
@RestController
@Validated
@RequestMapping(path = "/api/supplier-commande", produces = MediaType.APPLICATION_JSON_VALUE)
public class SupplierCommandeController {

    private final SupplierCommandeService service;

    public SupplierCommandeController(SupplierCommandeService service) {
        this.service = service;
    }

    // ===== Read =====
    @GetMapping
    public List<SupplierCommandeDto> list() {
        return service.findAll();
    }

    @GetMapping("/page")
    public Page<SupplierCommandeDto> listPage(
    ...
    @GetMapping("/sorted")
    public List<SupplierCommandeDto> listSorted(Sort sort) {
        return service.findAll(sort);
    }

    @GetMapping("/{id}")
    public SupplierCommandeDto get(@PathVariable Long id) {
        return service.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("supplierCommande not found with id=" + id));
    }

    @GetMapping("/uuid/{uuid}")
    public SupplierCommandeDto getByUuid(@PathVariable UUID uuid) {
        return service.findByUuid(uuid)
                .orElseThrow(() -> new ResourceNotFoundException("supplierCommande not found with uuid=" + uuid));
    }

    // ===== Create =====
    @PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<SupplierCommandeDto> create(@Valid @RequestBody SupplierCommandeDto dto) {
        SupplierCommandeDto created = service.create(dto);
    ...
    public SupplierCommandeDto update(@PathVariable Long id, @Valid @RequestBody SupplierCommandeDto dto) {
        return service.update(id, dto);
    }

    // ===== Patch (partial) =====
    @PatchMapping(path = "/{id}", consumes = MediaType.APPLICATION_JSON_VALUE)
    public SupplierCommandeDto patch(@PathVariable Long id, @RequestBody SupplierCommandeDto dto) {
