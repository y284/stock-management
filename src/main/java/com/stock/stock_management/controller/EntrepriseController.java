...
@RestController
@Validated
@RequestMapping(path = "/api/entreprise", produces = MediaType.APPLICATION_JSON_VALUE)
public class EntrepriseController {
    ...
    @GetMapping("/page")
    public Page<EntrepriseDto> listPage(
    ...
    @GetMapping("/sorted")
    public List<EntrepriseDto> listSorted(Sort sort) {
        return service.findAll(sort);
    }

    @GetMapping("/{id}")
    public EntrepriseDto get(@PathVariable Long id) {
        return service.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("entreprise not found with id=" + id));
    }

    @GetMapping("/uuid/{uuid}")
    public EntrepriseDto getByUuid(@PathVariable UUID uuid) {
        return service.findByUuid(uuid)
                .orElseThrow(() -> new ResourceNotFoundException("entreprise not found with uuid=" + uuid));
    }

    // ===== Create =====
    @PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<EntrepriseDto> create(@Valid @RequestBody EntrepriseDto dto) {
        EntrepriseDto created = service.create(dto);
    ...
    public EntrepriseDto patch(@PathVariable Long id, @RequestBody EntrepriseDto dto) {
