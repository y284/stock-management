...
@RestController
@Validated
@RequestMapping(path = "/api/sale-commande-line", produces = MediaType.APPLICATION_JSON_VALUE)
public class SaleCommandeLineController {

    private final SaleCommandeLineService service;

    public SaleCommandeLineController(SaleCommandeLineService service) {
        this.service = service;
    }

    // ===== Read =====
    @GetMapping
    public List<SaleCommandeLineDto> list() {
        return service.findAll();
    }

    @GetMapping("/page")
    public Page<SaleCommandeLineDto> listPage(
    ...
    @GetMapping("/sorted")
    public List<SaleCommandeLineDto> listSorted(Sort sort) {
        return service.findAll(sort);
    }

    @GetMapping("/{id}")
    public SaleCommandeLineDto get(@PathVariable Long id) {
        return service.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("saleCommandeLine not found with id=" + id));
    }

    @GetMapping("/uuid/{uuid}")
    public SaleCommandeLineDto getByUuid(@PathVariable UUID uuid) {
        return service.findByUuid(uuid)
                .orElseThrow(() -> new ResourceNotFoundException("saleCommandeLine not found with uuid=" + uuid));
    }

    // ===== Create =====
    @PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<SaleCommandeLineDto> create(@Valid @RequestBody SaleCommandeLineDto dto) {
        SaleCommandeLineDto created = service.create(dto);
    ...
    public SaleCommandeLineDto update(@PathVariable Long id, @Valid @RequestBody SaleCommandeLineDto dto) {
        return service.update(id, dto);
    }

    // ===== Patch (partial) =====
    @PatchMapping(path = "/{id}", consumes = MediaType.APPLICATION_JSON_VALUE)
    public SaleCommandeLineDto patch(@PathVariable Long id, @RequestBody SaleCommandeLineDto dto) {
