...
@RestController
@Validated
@RequestMapping(path = "/api/invoice-client", produces = MediaType.APPLICATION_JSON_VALUE)
public class InvoiceClientController {

    private final InvoiceClientService service;

    public InvoiceClientController(InvoiceClientService service) {
        this.service = service;
    }

    // ===== Read =====
    @GetMapping
    public List<InvoiceClientDto> list() {
        return service.findAll();
    }

    @GetMapping("/page")
    public Page<InvoiceClientDto> listPage(
    ...
    @GetMapping("/sorted")
    public List<InvoiceClientDto> listSorted(Sort sort) {
        return service.findAll(sort);
    }

    @GetMapping("/{id}")
    public InvoiceClientDto get(@PathVariable Long id) {
        return service.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("invoiceClient not found with id=" + id));
    }

    @GetMapping("/uuid/{uuid}")
    public InvoiceClientDto getByUuid(@PathVariable UUID uuid) {
        return service.findByUuid(uuid)
                .orElseThrow(() -> new ResourceNotFoundException("invoiceClient not found with uuid=" + uuid));
    }

    // ===== Create =====
    @PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<InvoiceClientDto> create(@Valid @RequestBody InvoiceClientDto dto) {
        InvoiceClientDto created = service.create(dto);
    ...
    public InvoiceClientDto update(@PathVariable Long id, @Valid @RequestBody InvoiceClientDto dto) {
        return service.update(id, dto);
    }

    // ===== Patch (partial) =====
    @PatchMapping(path = "/{id}", consumes = MediaType.APPLICATION_JSON_VALUE)
    public InvoiceClientDto patch(@PathVariable Long id, @RequestBody InvoiceClientDto dto) {
