...
import com.stock.stock_management.mapper.SupplierMapper;
...
public class SupplierServiceImpl implements SupplierService {
    ...
    public SupplierDto update(Long id, SupplierDto dto) {
        ...
        Supplier current = repository.findById(id)
            .orElseThrow(() -> new ResourceNotFoundException("supplier not found with id=" + id));

        precheckUpdate(id, dto);

        // Build a replacement from DTO
        Supplier replaced = mapper.toEntity(dto);
    ...
    @Override
    @Transactional
    public SupplierDto patch(Long id, SupplierDto dto) {
    ...
    @Override
    @Transactional(readOnly = true)
    public Optional<SupplierDto> findByUuid(UUID uuid) {
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SupplierDto> findAll(Sort sort) {
    ...
    private void guardDelete(Long id) {
        if (supplierCommandeRepository.countBySupplierId(id) > 0) { throw new ReferentialIntegrityException("supplier has dependent supplier_commande records"); }
    }
}
